...
const isMotoboyMiddleware = async (req, res, next) => {
  try {
    ...
    if (!motoboy) {
      return res.status(403).json({ message: 'Acesso permitido apenas para motoboys' });
    }
    ...
    return res.status(500).json({ message: 'Erro ao verificar perfil de motoboy', error: error.message });
  }
};
...
const isEstablishmentMiddleware = async (req, res, next) => {
  try {
    ...
    if (!user || !user.cnpj || !user.cnpj_approved) {
      return res.status(403).json({ message: 'Acesso permitido apenas para estabelecimentos aprovados' });
    }
    ...
    return res.status(500).json({ message: 'Erro ao verificar perfil de estabelecimento', error: error.message });
  }
};
...
router.put('/profile', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
...
router.post('/register', authenticateToken, async (req, res) => {
  try {
    ...
    if (!name || !phone || !cpf || !licensePlate || !vehicleModel || !vehicleYear) {
    ...
    if (existingMotoboy) {
      return res.status(400).json({ message: 'Você já possui um registro como motoboy' });
    }
    
    // Verificar se CPF ou placa já estão em uso
    const cpfExists = await Motoboy.findOne({ cpf });
    if (cpfExists) {
      return res.status(400).json({ message: 'CPF já está registrado no sistema' });
    }
    
    const licensePlateExists = await Motoboy.findOne({ licensePlate });
    if (licensePlateExists) {
      return res.status(400).json({ message: 'Placa já está registrada no sistema' });
    }
    ...
    res.status(500).json({ message: 'Erro ao registrar motoboy', error: error.message });
  }
});

// Enviar documentos
router.post('/documents', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
    const { type, url } = req.body;
    
    if (!type || !url) {
      return res.status(400).json({ message: 'Tipo e URL do documento são obrigatórios' });
    }
...
router.put('/location', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
    const { longitude, latitude } = req.body;
    
    if (!longitude || !latitude) {
      return res.status(400).json({ message: 'Coordenadas de longitude e latitude são obrigatórias' });
    }
...
router.put('/availability', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
    const { isAvailable } = req.body;
    
    if (typeof isAvailable !== 'boolean') {
      return res.status(400).json({ message: 'Status de disponibilidade deve ser um booleano' });
    }
    ...
    res.status(200).json({
      message: `Status de disponibilidade alterado para ${isAvailable ? 'disponível' : 'indisponível'}`,
      isAvailable: req.motoboy.isAvailable
    });
  } catch (error) {
    console.error('Erro ao atualizar disponibilidade:', error);
    res.status(500).json({ message: 'Erro ao atualizar disponibilidade', error: error.message });
  }
});

// Listar pedidos do motoboy
router.get('/orders', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
...
router.post('/accept-order/:orderId', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
    ...
    if (!order) {
      return res.status(404).json({ message: 'Pedido não encontrado ou não disponível' });
    }
    
    // Verificar se o motoboy está disponível
    if (!req.motoboy.isAvailable) {
      return res.status(400).json({ message: 'Você precisa estar disponível para aceitar pedidos' });
    }
    
    // Verificar se o motoboy está aprovado
    if (!req.motoboy.register_approved) {
      return res.status(403).json({ message: 'Seu registro como motoboy ainda não foi aprovado' });
    }
    ...
    if (order.customer.geolocation && req.motoboy.geolocation) {
      ...
      const estimatedTimeMinutes = geocodeService.estimateTravelTime(distance);
    ...
    res.status(500).json({ message: 'Erro ao aceitar pedido', error: error.message });
  }
});

// Completar entrega de um pedido
router.put('/complete-order/:orderId', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
    const orderId = req.params.orderId;
    ...
    if (!order) {
      return res.status(404).json({ message: 'Pedido não encontrado ou não está em entrega por você' });
    }
...
router.put('/cancel-order/:orderId', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
    const orderId = req.params.orderId;
...
router.get('/nearby', authenticateToken, isEstablishmentMiddleware, async (req, res) => {
  try {
    const { longitude, latitude, maxDistance = 5000, onlyAvailable = true } = req.query;
    ...
    const motoboys = await Motoboy.find(query)
      .select('name phone geolocation score profileImage isAvailable lastActive')
      .limit(10);
    ...
    const motoboyWithDistance = motoboys.map(motoboy => {
      ...
      return {
        ...
        estimatedTimeMinutes: geocodeService.estimateTravelTime(distanceMeters)
      };
    });
...
router.post('/request-delivery', authenticateToken, isEstablishmentMiddleware, async (req, res) => {
  try {
    const { orderId, motoboyId } = req.body;
    
    if (!orderId || !motoboyId) {
      return res.status(400).json({ message: 'ID do pedido e ID do motoboy são obrigatórios' });
    }
    ...
    if (!order) {
      return res.status(404).json({ message: 'Pedido não encontrado ou não está disponível para entrega' });
    }
    ...
    if (!motoboy) {
      return res.status(404).json({ message: 'Motoboy não encontrado ou não está disponível' });
    }
    ...
    res.status(500).json({ message: 'Erro ao solicitar entrega', error: error.message });
  }
});

// Obter estatísticas de entrega do motoboy
router.get('/stats', authenticateToken, isMotoboyMiddleware, async (req, res) => {
  try {
    ...
    const completedOrders = recentOrders.filter(order => order.status === 'entregue').length;
    const canceledOrders = recentOrders.filter(order => order.status === 'cancelado').length;
...
router.put('/approve/:motoboyId', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Erro ao aprovar motoboy', error: error.message });
  }
});
...
router.put('/reject/:motoboyId', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Erro ao rejeitar motoboy', error: error.message });
  }
});
...
router.post('/rate/:motoboyId', authenticateToken, async (req, res) => {
  try {
    ...
    if (!rating || !orderId) {
      return res.status(400).json({ message: 'Avaliação e ID do pedido são obrigatórios' });
    }
    ...
    if (!order) {
      return res.status(404).json({ message: 'Pedido não encontrado ou não foi entregue por este motoboy' });
    }
    ...
    if (order.cnpj !== user.cnpj) {
      return res.status(403).json({ message: 'Você não tem permissão para avaliar este motoboy para este pedido' });
    }
    ...
    res.status(500).json({ message: 'Erro ao avaliar motoboy', error: error.message });
  }
});
