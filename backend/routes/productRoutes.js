...
router.post('/create', authenticateToken, async (req, res) => {
  try {
    ...
    let product = await Product.findOne({ productName: productName, cnpj: user.cnpj });
    ...
    if (product) {
      ...
      product.superPromo = superPromo !== undefined ? superPromo : product.superPromo;
      await product.save();
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const product = await Product.findOne({ _id: req.params.id, cnpj: user.cnpj });
    if (!product) {
...
router.get('/public/all', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Erro ao listar produtos públicos', error: error.message });
  }
});
