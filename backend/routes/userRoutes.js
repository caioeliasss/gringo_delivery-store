...
router.get('/nearby', authenticateToken, async (req, res) => {
  try {
    ...
    const nearbyEstablishments = await User.find({
      cnpj_approved: true,
      isAvailable: true,
      geolocation: {
        $near: {
    ...
    .select('displayName businessName address phone geolocation businessHours photoURL');
    
    res.status(200).json(nearbyEstablishments);
  } catch (error) {
    console.error('Erro ao buscar estabelecimentos próximos:', error);
    res.status(500).json({ message: 'Erro ao buscar estabelecimentos próximos', error: error.message });
  }
});
...
router.put('/approve-cnpj/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    if (!user.cnpj) {
      return res.status(400).json({ message: 'Usuário não possui CNPJ cadastrado' });
    }
    ...
    res.status(500).json({ message: 'Erro ao aprovar CNPJ', error: error.message });
  }
});
...
router.put('/availability', authenticateToken, async (req, res) => {
  try {
    ...
    if (isAvailable === undefined) {
      return res.status(400).json({ message: 'Estado de disponibilidade não fornecido' });
    }
    ...
    res.status(200).json({ 
      message: `Estabelecimento agora está ${isAvailable ? 'aberto' : 'fechado'}`,
...
router.get('/public/:id', async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .select('displayName businessName address phone geolocation businessHours photoURL isAvailable');
    ...
    if (!user.cnpj_approved) {
      return res.status(404).json({ message: 'Estabelecimento não encontrado ou não aprovado' });
    }
    
    res.status(200).json(user);
  } catch (error) {
    console.error('Erro ao buscar estabelecimento:', error);
    res.status(500).json({ message: 'Erro ao buscar estabelecimento', error: error.message });
  }
});
