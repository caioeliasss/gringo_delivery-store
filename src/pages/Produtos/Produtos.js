  ...
  useEffect(() => {
    const fetchProdutos = async () => {
      try {
        ...
        console.error("Erro ao carregar produtos:", err, error);
        setError(
    ...
    if (filterImage === "with") {
      result = result.filter(
        (produto) =>
          produto.image &&
          produto.image !==
            "https://www.svgrepo.com/show/491915/food-color-pizza-slice.svg"
      );
  ...
  const handleSubmitProduto = async (e) => {
    ...
    try {
      ...
      const produtoData = {
        ...
        priceOnSale: produtoForm.priceOnSale
          ? parseFloat(produtoForm.priceOnSale)
          : undefined,
      };
      ...
      if (currentProduto) {
        ...
        setProdutos((prev) =>
          prev.map((p) =>
            p._id === currentProduto._id ? response.data.product : p
          )
        );
    ...
    >
      ...
      >
        <Container maxWidth="lg">
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} md={3}>
                  <FormControl fullWidth size="small">
                    <InputLabel id="image-filter-label">
                      Filtrar por imagem
                    </InputLabel>
                    <Select
          ...
          <Box sx={{ mb: 2 }}>
            <Typography variant="body2" color="text.secondary">
              Exibindo {filteredProdutos.length} produto
                    ...
                    <CardContent sx={{ flexGrow: 1 }}>
                      ...
                      <Box sx={{ display: "flex", alignItems: "center" }}>
                        <Typography
                          ...
                          sx={{
                            textDecoration:
                              produto.priceOnSale > 0 ? "line-through" : "none",
                            color:
